...
```{r}
trust_placement_image <- trust_2023_raw_data %>%
...
```{r}
trust_placement_image_percentages <- trust_placement_image %>%
  group_by(General) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate(percentage = count / sum(count) * 100)
trust_placement_image_percentages
...
merged_vis <- trust_2023_raw_data %>%
  left_join(trust_placement_image, by = "image_new") %>%
  group_by(image_new) %>%
