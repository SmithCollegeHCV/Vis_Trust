...
merged_individual <- trust_2023_raw_data %>%
  ...
  group_by(session_id) %>%
  select(session_id, General, sex, age, education, mini_score)%>%
  filter(General %in% c("higher", "lower"))%>%
...
```{r}
individual_char_prelim_tree <- rpart(General ~.-session_id, data = merged_individual, method="class", minsplit = 8)
```
