...
```{r}
vis_attr_prelim_tree <- rpart(General ~.-image_new , data = merged_trust, method="class", minsplit = 8)
```
...
```
```{r}
merged_pt2 <- trust_2023_raw_data %>%
  left_join(individual_averages, by = "session_id") %>%
  group_by(session_id) %>%
  select(session_id, Familiarity, Credibility, Clarity, Confidence, Reliability, sex, age, education, mini_score)%>%
  slice(1)  
