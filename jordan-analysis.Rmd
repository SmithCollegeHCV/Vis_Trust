...
belief_rf <- randomForest(response ~.-image_new, data = strongly_believe_data, mtry= 12, importance = TRUE) 
...
clear_rf <- randomForest(response ~.-image_new, data = strongly_clear_data, mtry= 12, importance = TRUE) 
varImpPlot(clear_rf)
```
...
reliable_rf <- randomForest(response ~.-image_new, data = strongly_reliable_data, mtry= 12, importance = TRUE) 
varImpPlot(reliable_rf)
```
...
familiar_rf <- randomForest(response ~.-image_new, data = strongly_familiar_data, mtry= 12, importance = TRUE) 
varImpPlot(familiar_rf)
```
...
confidence_rf <- randomForest(response ~.-image_new, data = strong_confidence_data, mtry= 12, importance = TRUE) 
...
```
Vistype is an important predictor for accuracy, purity, and the desicion tree therefore the tree is representative.
...
```
There is no agreement between accuracy and purity which indicates overfitting in our tree
...
varImpPlot(belief_rf)
```
We use random forest to see what predictors affect accuracy and purity. Data literacy and vistype are the most important feature and this confirms that the tree is not over_fitting. We find that age doesn't appear to be the strong predictor in the tree but is the most important factor in purity.
...
```{r}
strongly_clear_data$response = factor(strongly_clear_data$response) 
clarity_rf <- randomForest(response ~.-image_new, data = strongly_clear_data, mtry= 12, importance = TRUE) 
varImpPlot(clarity_rf)
```
Data literacy and human recognizable object are the most important feature and this confirms that the tree is not over_fitting. We find that age is not a predictor in the tree but is an important factor in purity.
...
```{r}
strongly_reliable_data$response = factor(strongly_reliable_data$response) 
reliability_rf <- randomForest(response ~.-image_new, data = strongly_reliable_data, mtry= 12, importance = TRUE) 
varImpPlot(reliability_rf)
```
- Vistype in an important factor both in the tree for accuracy and purity. 
- Age appears to be important in the tree for purity but not for accuracy
- in vice versa for attribute black white
...
```{r}
strongly_familiar_data$response = factor(strongly_familiar_data$response) 
familiarity_rf <- randomForest(response ~.-image_new, data = strongly_familiar_data, mtry= 12, importance = TRUE) 
varImpPlot(familiarity_rf)
```
- tree is over-fitting because the most important factors for accuracy, ie human depiction and data in ratio, are not significant predictor in the tree
...
varImpPlot(confidence_rf)
```
- we suspect that it is over fitted because vis type and age are important predictors in the tree bu tnot in accuracy.
