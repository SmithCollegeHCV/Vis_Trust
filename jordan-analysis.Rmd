...
```{r}
individual_char_prelim_tree <- rpart(General ~.-session_id -sex -age, data = merged_individual, method="class", minsplit=4)
```
...
From the decision tree we find that data literacy score is the biggest predictor for individual trust.
...
```
Mini score is important for both accuracy and purity.
