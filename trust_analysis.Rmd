...
believe_real_data <- trust_2023_raw_data %>%
  ...
  select(-c(...1, subfolder, session_id, category, question_type, cluster, image, image_new, title, likertscale, starts_with("mem"))) %>% head(600)
```
...
```{r}
believe_prelim_tree <- na.omit(believe_real_data)
#believe_prelim_tree$name <- droplevels(believe_prelim_tree$name, exclude = "")

bag_credibility_bagged = randomForest(response ~ ., 
